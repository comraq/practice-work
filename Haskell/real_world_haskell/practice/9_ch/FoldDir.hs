...
foldTreeOrder :: ([FilePath] -> [FilePath]) -> Iterator a -> a -> FilePath -> IO a
foldTreeOrder order iter initSeed path = do
    endSeed <- fold initSeed path
    return $ unwrap endSeed
  where fold seed subpath = order <$> getUsefulContents subpath >>= walk seed
        walk seed (name:names) = do
          let path' = path </> name
          info <- getInfo path'
          case iter seed info of
