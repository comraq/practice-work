...
type DbName = String
-- | Initilialize DB and return database Connection
connect :: DbName -> IO Connection
connect dbName = do
  dbh <- connectMySQL $ dbInfo { mysqlDatabase = dbName }
  prepDB dbh
  return dbh

{- | Prepare the database for our data.

We create two tables and ask the database engine to verify some pieces of data
consistency for us:

* castid and epid both are unique primary keys and must never be duplicated
* castURL is also unique
...
prepDB dbh = do
  ...
  when (not ("podcasts" `elem` tables)) $
    do run dbh "CREATE TABLE podcasts (\
               \castid INTEGER PRIMARY KEY AUTO_INCREMENT,\
               \castURL TEXT NOT NULL,\
               \UNIQUE(castURL(30)))" []
       return ()

  when (not ("episodes" `elem` tables)) $
    do run dbh "CREATE TABLE episodes (\
               \epid INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT,\
               \epcastid INTEGER NOT NULL,\
               \epurl TEXT NOT NULL,\
               \epdone INTEGER NOT NULL,\
               \UNIQUE(epcastid, epurl(30)),\
...
addPodcast dbh podcast = handleSql errorHandler $ do
    {-
     - Insert the castURL into the table. The database will automatically
     - assign a cast ID.
     -}
    ...
    case r of
      [[ id ]] -> return $ podcast { castId = fromSql id }
...
simply ignore request to add duplicate episodes. This way, when we are
processing a feed, each URL encountered can be fed to this function,
without having to first look it up in the DB.

Also, we generally won't care about the new ID here, so do not bother
fetch it.
-}
addEpisode :: IConnection conn => conn -> Episode -> IO ()
addEpisode dbh ep =
  run dbh "INSERT IGNORE INTO episodes (epCastId, epURL, epDone)\
          \VALUES (?, ?, ?)"
          [toSql (castId $ epCast ep), toSql $ epURL ep, toSql $ epDone ep]
  >> return ()

{- | Modifies an existing podcast. Looks up the given podcast by ID and
modifies the database record to match the passed Podcast.
-}
...
{- | Remove a podcast. First removes any episodes that may exist for this
podcast.
-}
...
{- | Get a particular podcast. Nothing if the ID doesn't match, or Just
Podcast if it does.
-}
...
getPodcast dbh wantedId = do
  ...
  case res of
    ...
    []    -> return Nothing
    _     -> fail $ "Really bad error; more than one podcast with ID"
...
convPodcastRow err = error $ "Can't convert podcast row " ++ show err
