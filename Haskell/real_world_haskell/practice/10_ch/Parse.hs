...
firstParser ==> secondParser = Parse chainedParser
  where chainedParser initState = case runParse firstParser initState of
...
{-
 - Explanation:
 -   At each iteration, parseWhile calls parseByte, pulling out the byte and
 - concatenating to the recursed results.
 -
 -   If parse fails (Nothing) or predicate returns false, we return '[]' as
 - the resulting parsed value, which will be cons with each byte in previous
 - recursion calls.
 -
 -   'identity' will return a tuple with the concatenated parsed result as
 - #1/fst and the remaining state as #2/snd
 -
 -}
...
parseAsciiPGM :: Parse Greymap
parseAsciiPGM = parseWhileWith w2c notWhite ==> \header -> skipSpaces
...
parseNats n = parseNat ==> (L.cons . fromIntegral >>> (<$> parseNats (n - 1)))
