...
main = do
  ...
  forM_ files $ \file -> do
    words <- timed "read words" $ BS.lines <$> BS.readFile file
    ...
    filt <- timed "construct filter" $ case B.easyList errRate words of
      Left errmsg -> putStrLn ("Error: " ++ errmsg) >> exitFailure
