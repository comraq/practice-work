...
countTree             :: Rose a -> Int -> Int
countTree (_ :> []) _ =  1
countTree (_ :> cs) x =  x + foldr (\c acc -> acc + countTree c x) 0 cs 

size   :: Rose a -> Int
size t =  countTree t 1

leaves   :: Rose a -> Int
leaves t =  countTree t 0
...
instance Functor Rose where
  fmap f (l :> []) =  f l :> []
  fmap f (r :> cs) =  f r :> foldr (\r acc -> fmap f r : acc) [] cs
...
instance Foldable Rose where
  fold (r :> cs) = mappend r $ foldr (\t acc -> mappend acc $ fold t) mempty cs
