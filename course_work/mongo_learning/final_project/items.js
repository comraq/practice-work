...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        (function getNext() {
          cursor.next().then(function success(doc) {
            if (!doc) {
              // doc is null, we have reached the end
              // of the aggregate cursor
              categories.splice(0, 0, { _id: "All", num: sum });
              callback(categories);
              return;
            }
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        cursor.toArray().then(function success(docs) {
          callback(docs);
        }, function error(err) {
          callback(null);
        });
    ...
    this.getNumItems = function(category, callback) {
        ...
        this.db.collection("item").count(filter)
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        this.db.collection("item").count({ $text: { $search: query } })
          .then(function success(count) {
            callback(count);
